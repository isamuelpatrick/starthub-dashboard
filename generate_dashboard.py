...
.filter-pill{{display:flex;align-items:center;gap:6px;background:var(--surface2);border:1px solid var(--border2);border-radius:6px;padding:4px 10px}}
.filter-pill label{{font-size:.7rem;font-weight:600;color:var(--muted);white-space:nowrap}}
...
<div id="header">
  ...
  <div class="header-filters">
    <div class="filter-pill"><label>Year</label><select id="filter-year"><option value="all">All Years</option></select></div>
    <div class="filter-pill"><label>Country</label><select id="filter-country"><option value="all">All Countries</option><option value="Uganda">Uganda</option><option value="Tanzania">Tanzania</option><option value="Kenya">Kenya</option></select></div>
  </div>
...
const state = {{
  ...
  drillSearch:'', drillSelected:null,   // null = all selected
}};
...
function getFiltered(){{
  if(state.year==='all'){{
    if(state.country==='all') return DATA.overall.all||{{}};
    return DATA.overall.countries?.[state.country]||{{}};
  }}
  return getMetrics(state.year, state.country)||{{}};
}}
...
function getProjects(drillCountry){{
  const years = state.year==='all'?getYears():[state.year];
  const map = {{}};   // key → entry
  const display = {{}}; // key → canonical display name
...
function buildTrendDatasets(){{
  ...
  return {{
    ...
    datasets: METRICS.filter(m=>state.activeMetrics.includes(m.key)).map(m=>{{
      const vals = years.map(yr=>{{const mm=getMetrics(yr,state.country);return mm?mm[m.key]||0:0;}});
      return {{
...
function renderDonut(){{
  const yrs = state.year==='all'?getYears():[state.year];
  const vals = COUNTRIES.map(c=>yrs.reduce((s,yr)=>{{const m=getMetrics(yr,c);return s+(m?.trained_total||0);}},0));
  const ctx = document.getElementById('donut-chart');
  if(charts.donut) charts.donut.destroy();
  charts.donut = new Chart(ctx,{{
    type:'doughnut',
    data:{{labels:COUNTRIES,datasets:[{{
      data:vals, backgroundColor:COUNTRIES.map(c=>COUNTRY_BG[c]),
      borderColor:COUNTRIES.map(c=>COUNTRY_COLORS[c]), borderWidth:2, hoverOffset:8
    }}]}},
    options:{{
...
function renderCountryTable(){{
  ...
  // Max per metric across countries
  const maxes = {{}};
  visibleMetrics.forEach(met=>{{
    maxes[met.key]=Math.max(...COUNTRIES.map(c=>cData[c][met.key]||0),1);
  }});

  let html=`<div style="overflow-x:auto"><table class="country-metrics-table">
    <thead><tr>
      <th style="min-width:160px">Metric</th>
      ${{COUNTRIES.map(c=>`<th style="min-width:140px"><div class="country-col-head">
...
function renderGender(){{
  const years = getYears();
  const mVals = years.map(yr=>{{const m=getMetrics(yr,state.country);return m?.trained_m||0;}});
  const fVals = years.map(yr=>{{const m=getMetrics(yr,state.country);return m?.trained_f||0;}});
...
function renderGrants(){{
  const ps = getProjects(state.country).filter(p=>(p.metrics.grants_value||0)>0)
...
function openModal(yr){{
  ...
  document.getElementById('modal-sub').textContent=`${{c==='all'?'All Countries':c}} · Click outside to close`;
...
function init(){{
  ...
  getYears().forEach(yr=>{{ const o=document.createElement('option'); o.value=yr; o.textContent=yr; sel.appendChild(o); }});
  sel.addEventListener('change',e=>{{ state.year=e.target.value; renderAll(); }});
  document.getElementById('filter-country').addEventListener('change',e=>{{ state.country=e.target.value; renderAll(); }});
